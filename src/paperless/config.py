...
class AIConfig(BaseConfig):
    ...
    _ai_enabled: bool = dataclasses.field(init=False)
    ...
    llm_timeout: str = dataclasses.field(init=False)

    def __post_init__(self) -> None:
        app_config = self._get_config_instance()
        self._ai_enabled = app_config.ai_enabled or settings.AI_ENABLED
        ...
        self.llm_allow_internal_endpoints = getattr(settings, "LLM_ALLOW_INTERNAL_ENDPOINTS", True)
        self.llm_timeout = os.getenv("PAPERLESS_AI_LLM_TIMEOUT", "120")

    @property
    def ai_enabled(self) -> bool:
        if os.getenv("PAPERLESS_AI_DISABLED", "false").lower() == "true":
            return False
