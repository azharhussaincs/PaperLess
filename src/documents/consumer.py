...
from paperless_ai.ai_classifier import get_ai_document_classification_from_text


class AIProcessingPlugin(
    AlwaysRunPluginMixin,
    NoSetupPluginMixin,
    NoCleanupPluginMixin,
    LoggingMixin,
    ...
    def run(self) -> str | None:
        ...
        if not self.metadata.content:
            return "No content for AI processing"

        # Zero-config auto-detection: check if Ollama is available
        from paperless_ai.resolver import get_ollama_endpoint
        if not get_ollama_endpoint(ai_config.llm_endpoint):
            self.log.debug("Ollama not detected, skipping AI processing.")
            return "AI processing skipped (Ollama not found)"

        try:
            suggestions = get_ai_document_classification_from_text(
                text=self.metadata.content,
                filename=self.metadata.filename or self.input_doc.original_file.name,
            )

            # Apply suggestions to metadata overrides
            if suggestions.get("title") and not self.metadata.title:
                self.metadata.title = suggestions["title"]

            # Note: Tags and other metadata can be added here as well
            # but for safety in MVP we focus on title/summary first.

            return "AI processing completed"
        except Exception as e:
            self.log.warning(f"AI processing failed (Ollama/LLM): {e}")
